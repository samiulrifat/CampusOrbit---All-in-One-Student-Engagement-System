...
async function createClub(req, res) {
  ...
  try {
    ...
    await newClub.save();
    res.status(201).json({ message: 'Club created', club: newClub, user });
  } catch (err) {
...
async function getClubs(req, res) {
  try {
    const clubs = await Club.find({}).populate('members.userId', 'name email');
...
async function inviteMember(req, res) {
  ...
  try {
    ...
    res.json({ message: 'User added to club', club, user });
  } catch (err) {
